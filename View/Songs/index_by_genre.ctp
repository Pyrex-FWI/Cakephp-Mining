 ...
 **************************************************/
 ?>
<div class="list_genre" style="height:200px;overflow: hidden;overflow-y: scroll;">
<?php
//debug(count($NbSongsByGenre));
$nbperListCol = ceil(count($NbSongsByGenre)/5);
$c=1;
$list = array();
foreach($NbSongsByGenre as $genreData):
 	$list[] = $this->Html->link($this->Text->truncate($genreData['Genre']['name'],20),array('action'=>'indexByGenre',$genreData['Genre']['id']),array('title'=>$genreData['Genre']['name'],'onclick'=>"load_page('#content',this.href);return false;"))." (".$genreData[0]['nb'].")";
endforeach; 
$lists = array_chunk($list, $nbperListCol);
//debug($lists);
foreach($lists as $ul):
	echo "\n<ul style='float:left;'>\n";
	foreach($ul as $li):
