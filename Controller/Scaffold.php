 ...
 */
	public function __construct(Controller $controller, CakeRequest $request) {
		...
		$scaffoldFields = $this->controller->getColGrid();
		$tmp = array();
		foreach(array_keys($scaffoldFields) as $m){
			 $tmp = array_merge($tmp,array_keys($scaffoldFields[$m]));
		}
		$scaffoldFields = $tmp; unset($tmp);
